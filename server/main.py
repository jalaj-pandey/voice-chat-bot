...
async def chat_with_customer(user_query: UserQuery):
    if not user_query.hotel_name or not user_query.message:
        return JSONResponse(content={"error": "Missing hotel_name or message in request."}, status_code=400)

    hotel = next((h for h in hotels_list if h["hotel_name"].lower() == user_query.hotel_name.lower()), None)
    if not hotel:
        return JSONResponse(content={"error": f"Hotel '{user_query.hotel_name}' not found."}, status_code=404)
    ...
    if "check-in" in user_query.message.lower() or "20" in user_query.message:
        booking_state["check_in_date"] = "20 July"
        booking_state["check_out_date"] = "25 July"
    if "2 persons" in user_query.message.lower() or "2 guests" in user_query.message.lower():
        booking_state["guests"] = "2"
    if "cottage" in user_query.message.lower():
        booking_state["room_type"] = "Standard Cottage"
    if "valley" in user_query.message.lower():
